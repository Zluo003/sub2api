...
// 与后端 service.BillingMode* 对齐：'video' 是上游 Grok 的按视频计费，
// 'video_duration' 是本仓库 Seedance 网关的按秒计费，两者并存。
export const BILLING_MODE_VIDEO = 'video'
export const BILLING_MODE_VIDEO_DURATION = 'video_duration'
...
export function getBillingModeLabel(mode: string | null | undefined, t: (key: string) => string): string {
  switch (mode) {
    ...
    case BILLING_MODE_VIDEO_DURATION: return t('admin.usage.billingModeVideoDuration')
...
export function getBillingModeBadgeClass(mode: string | null | undefined): string {
  switch (mode) {
    ...
    case BILLING_MODE_VIDEO_DURATION: return 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-300'
...
export function isImageUsage(row: Pick<ImageBillingRow, 'image_count' | 'billing_mode'> | null | undefined): boolean {
  return (row?.image_count ?? 0) > 0 &&
    ...
    row?.billing_mode !== BILLING_MODE_VIDEO_DURATION
}
