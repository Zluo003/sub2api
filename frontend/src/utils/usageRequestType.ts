...
const VALID_REQUEST_TYPES = new Set<UsageRequestType>(['unknown', 'sync', 'stream', 'ws_v2', 'cyber', 'video'])
...
export const requestTypeToLegacyStream = (requestType?: UsageRequestType | null): boolean | null | undefined => {
  ...
  if (!requestType || requestType === 'unknown' || requestType === 'cyber' || requestType === 'video') {
    return null
  }
  if (requestType === 'sync') {
    return false
