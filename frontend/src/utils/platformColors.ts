...
export type Platform = 'anthropic' | 'openai' | 'antigravity' | 'gemini' | 'grok' | 'video'
...
const ACCENT_BAR: Record<Platform, string> = {
  ...
  video: 'bg-gradient-to-r from-cyan-400 to-teal-500',
}
...
const BUTTON: Record<Platform, string> = {
  ...
  video: 'bg-cyan-600 text-white hover:bg-cyan-700 active:bg-cyan-800 dark:bg-cyan-600/80 dark:hover:bg-cyan-500',
}
...
const GRADIENT: Record<Platform, string> = {
  ...
  video: 'from-cyan-500 to-teal-600',
}
...
function isPlatform(p: string): p is Platform {
  return p === 'anthropic' || p === 'openai' || p === 'antigravity' || p === 'gemini' || p === 'grok' || p === 'video'
}
