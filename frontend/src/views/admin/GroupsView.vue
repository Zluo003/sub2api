  ...
  <AppLayout>
      ...
      <template #table>
        ...
        >
          <template #cell-name="{ value, row }">
            <div class="space-y-1">
              <div class="flex items-center gap-2">
                ...
                <span
                  v-if="row.kind === 'agent'"
                  class="inline-flex items-center rounded bg-cyan-100 px-1.5 py-0.5 text-[10px] font-semibold text-cyan-800 dark:bg-cyan-900/40 dark:text-cyan-300"
                >
                  {{ t("admin.groups.agent.systemBadge") }}
                </span>
              </div>
              <div
                v-if="row.kind === 'agent'"
              ...
              >
                <span>{{ t("admin.groups.agent.systemCode") }}</span>
                <code>{{ row.system_code }}</code>
              </div>
            </div>
          </template>
          ...
          <template #cell-platform="{ value, row }">
            <span
              v-if="row.kind === 'agent'"
              class="inline-flex items-center rounded bg-gray-900 px-2.5 py-0.5 text-xs font-medium text-white dark:bg-gray-100 dark:text-gray-900"
            >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <span
                v-else
                class="flex flex-col items-center gap-0.5 p-1.5 text-gray-400 dark:text-gray-500"
                :title="t('admin.groups.agent.protectedHint')"
              >
                <Icon name="lock" size="sm" />
                <span class="text-xs">{{ t("admin.groups.agent.protected") }}</span>
              </span>
            </div>
          </template>
    ...
    >
      ...
      >
        ...
        <!-- 视频计费规则（仅 video 平台） -->
        <div v-if="createForm.platform === 'seedance'" class="border-t pt-4">
          <div class="mb-3 flex items-center justify-between gap-3">
            <div>
            ...
            <button
              ...
              @click="createForm.video_pricing_rules = defaultVideoPricingRules()"
            >
              {{ t("admin.groups.videoPricing.resetDefaults") }}
            </button>
          </div>
          <div class="space-y-3">
            <div
              v-for="(rule, index) in createForm.video_pricing_rules"
              :key="`create-video-${rule.model_code}-${rule.resolution}`"
              class="grid grid-cols-1 gap-3 rounded-lg border border-gray-200 p-3 dark:border-dark-700 md:grid-cols-[1.5fr_0.8fr_1fr_auto]"
            >
              <div>
                <label class="input-label">{{ t("admin.groups.videoPricing.model") }}</label>
                <div class="input flex items-center">{{ videoModelLabel(rule.model_code) }}</div>
              </div>
              <div>
                <label class="input-label">{{ t("admin.groups.videoPricing.resolution") }}</label>
                <div class="input flex items-center">{{ videoResolutionLabel(rule.resolution) }}</div>
              </div>
              <div>
                <label class="input-label">{{ t("admin.groups.videoPricing.creditsPerSecond") }}</label>
                <input
                  v-model.number="createForm.video_pricing_rules[index].credits_per_second"
                  type="number"
                  min="0"
                  step="0.0001"
                  class="input"
              ...
              <label class="flex items-end gap-2 pb-2 text-sm text-gray-700 dark:text-gray-300">
                <input
                  v-model="createForm.video_pricing_rules[index].enabled"
                  type="checkbox"
    ...
    >
      ...
      >
        <div
          ...
          class="flex gap-3 border-l-4 border-cyan-500 bg-cyan-50 px-4 py-3 text-cyan-950 dark:bg-cyan-950/30 dark:text-cyan-100"
        >
          <Icon name="shield" size="md" class="mt-0.5 flex-shrink-0" />
          <div>
            <p class="text-sm font-semibold">{{ t("admin.groups.agent.editTitle") }}</p>
            <p class="mt-1 text-xs leading-5 text-cyan-800 dark:text-cyan-300">
              {{ t("admin.groups.agent.editDescription") }}
            </p>
          </div>
        </div>
        <div>
        ...
        <div v-if="editForm.subscription_type !== 'subscription'">
          ...
          <div class="flex items-center gap-3">
            <button
              type="button"
              :disabled="editingGroup.kind === 'agent'"
              @click="
                editingGroup.kind !== 'agent' &&
        ...
        >
          <div class="mb-3 flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              class="btn btn-secondary btn-sm"
              @click="editForm.video_pricing_rules = defaultVideoPricingRules()"
            >
              {{ t("admin.groups.videoPricing.resetDefaults") }}
            </button>
          </div>
          <div class="space-y-3">
            <div
              v-for="(rule, index) in editForm.video_pricing_rules"
              :key="`edit-video-${rule.model_code}-${rule.resolution}`"
              class="grid grid-cols-1 gap-3 rounded-lg border border-gray-200 p-3 dark:border-dark-700 md:grid-cols-[1.5fr_0.8fr_1fr_auto]"
            >
              <div>
                <label class="input-label">{{ t("admin.groups.videoPricing.model") }}</label>
              ...
              <div>
                <label class="input-label">{{ t("admin.groups.videoPricing.creditsPerSecond") }}</label>
                <input
                  v-model.number="editForm.video_pricing_rules[index].credits_per_second"
                  type="number"
                  min="0"
                  step="0.0001"
                  class="input"
              ...
              <label class="flex items-end gap-2 pb-2 text-sm text-gray-700 dark:text-gray-300">
                <input
                  v-model="editForm.video_pricing_rules[index].enabled"
                  type="checkbox"
                  class="rounded border-gray-300 text-cyan-600 focus:ring-cyan-500"
                />
                {{ t("admin.groups.videoPricing.enabled") }}
          ...
          >
            ...
            <div class="flex-1">
              <div class="font-medium text-gray-900 dark:text-white">
                ...
                <span
                  v-if="group.kind === 'agent'"
                  class="ml-2 inline-flex rounded bg-cyan-100 px-1.5 py-0.5 text-[10px] font-semibold text-cyan-800 dark:bg-cyan-900/40 dark:text-cyan-300"
                >
                  {{ t("admin.groups.agent.systemBadge") }}
                </span>
              </div>
              <div class="text-xs text-gray-500 dark:text-gray-400">
                <span
                  v-if="group.kind === 'agent'"
                  class="inline-flex items-center rounded bg-gray-900 px-2 py-0.5 text-xs font-medium text-white dark:bg-gray-100 dark:text-gray-900"
                >
                  {{ t("admin.groups.agent.multiPlatform") }}
                </span>
                <span
...
const copyAccountsGroupOptionsForEdit = computed(() => {
  ...
  return eligibleGroups.map((g) => ({
    value: g.id,
    // 系统 Agent 分组会聚合多平台来源，标签上补出平台名以便区分
    label: editingAgentGroup
      ? `${g.name} · ${t("admin.groups.platforms." + g.platform)} (${g.account_count || 0} 个账号)`
      : copyAccountsGroupLabel(g),
  }));
});
...
const createForm = reactive({
  ...
  // 视频计费规则（仅 video 平台）
  video_pricing_rules: [] as VideoGroupPricingRule[],
...
const ensureVideoPricingRules = (rules: VideoGroupPricingRule[] | undefined): VideoGroupPricingRule[] => {
  ...
  const byKey = new Map<string, VideoGroupPricingRule>();
  for (const rule of incoming) {
    byKey.set(`${rule.model_code}:${rule.resolution}`, {
      model_code: rule.model_code,
      resolution: rule.resolution,
      credits_per_second: Number(rule.credits_per_second ?? 0),
      reference_video_multiplier: Number(rule.reference_video_multiplier ?? 1),
      enabled: rule.enabled ?? true,
    });
  }
  return defaultVideoPricingRules().map((rule) => {
    const existing = byKey.get(`${rule.model_code}:${rule.resolution}`);
    return existing ? { ...rule, ...existing } : rule;
  });
};

const serializeVideoPricingRules = (rules: VideoGroupPricingRule[]): VideoGroupPricingRule[] =>
  ensureVideoPricingRules(rules).map((rule) => ({
    model_code: rule.model_code,
    resolution: rule.resolution,
    credits_per_second: Number(rule.credits_per_second) || 0,
    reference_video_multiplier: Number(rule.reference_video_multiplier) || 1,
...
const videoModelLabel = (model: string) =>
  videoModelOptions.find((item) => item.value === model)?.label || model;

const videoResolutionLabel = (resolution: string) =>
  videoResolutionOptions.find((item) => item.value === resolution)?.label || resolution;
...
const openCreateModal = () => {
  if (createForm.platform === "seedance" && createForm.video_pricing_rules.length === 0) {
    createForm.video_pricing_rules = defaultVideoPricingRules();
  }
...
const closeCreateModal = () => {
  ...
  createForm.video_pricing_rules = [];
    ...
    const requestData = {
      ...
      video_pricing_rules:
        createForm.platform === "seedance"
          ? serializeVideoPricingRules(createForm.video_pricing_rules)
          : undefined,
...
const handleUpdateGroup = async () => {
  ...
  try {
    ...
    if (editingAgentGroup) {
      ...
      payload.profit_control_enabled = undefined;
      payload.profit_min_margin = undefined;
      payload.profit_safety_buffer = undefined;
    }
...
watch(
  ...
  (newVal) => {
    ...
    if (newVal === "seedance" && createForm.video_pricing_rules.length === 0) {
      createForm.video_pricing_rules = defaultVideoPricingRules();
