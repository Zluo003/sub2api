    ...
    <div class="yingzo-admin space-y-8">
      <header class="flex flex-col justify-between gap-4 border-b border-gray-200 pb-6 dark:border-dark-700 md:flex-row md:items-end">
        <div>
          ...
          <p class="mt-2 max-w-3xl text-sm leading-6 text-gray-500 dark:text-gray-400">管理公开域名、私有安装包、校验信息和当前发布版本。安装包保存在服务器数据目录，不进入公开 Git 仓库。</p>
        </div>
      ...
      <section>
        <div class="mb-4 flex items-center justify-between">
          <div>
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">同一时间只有一个已发布版本。回滚会把目标历史版本重新设为当前版本。</p>
          </div>
        ...
        <div class="overflow-x-auto border-y border-gray-200 dark:border-dark-700">
          <table class="w-full min-w-[980px] text-left text-sm">
            <thead class="bg-gray-50 text-xs text-gray-500 dark:bg-dark-900 dark:text-gray-400">
              <tr><th class="px-4 py-3">版本</th><th class="px-4 py-3">状态</th><th class="px-4 py-3">兼容性</th><th class="px-4 py-3">SHA-256</th><th class="px-4 py-3">大小</th><th class="px-4 py-3">更新时间</th><th class="px-4 py-3 text-right">操作</th></tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-dark-800">
              <tr v-for="release in releases" :key="release.id" class="align-top">
                ...
                <td class="max-w-[260px] px-4 py-4"><code class="block truncate text-xs" :title="release.sha256">{{ release.sha256 }}</code><span class="mt-1 block text-xs text-emerald-600">服务端已校验</span></td>
                <td class="px-4 py-4 text-xs text-gray-500">{{ formatBytes(release.size_bytes) }}</td>
              ...
              <tr v-if="!loading && releases.length === 0"><td colspan="7" class="px-4 py-12 text-center text-gray-500">尚未上传 Yingzo 安装包</td></tr>
            </tbody>
          </table>
        </div>
      </section>
...
import {
  ...
  type YingzoAdminSettings, type YingzoReleaseSummary,
} from '@/api/yingzo'
...
async function uploadRelease() {
  if (!selectedFile.value) { showMessage('请选择 .tar.gz 安装包。', 'error'); return }
  uploading.value = true
  const form = new FormData()
  form.set('package', selectedFile.value)
