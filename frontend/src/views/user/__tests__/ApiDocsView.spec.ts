...
describe('user ApiDocsView Seedance reference', () => {
  ...
  it('renders the detailed Chinese Seedance request and lifecycle contract', () => {
    ...
    expect(text).toContain('本地媒体上传与请求体自动转写')
    expect(text).toContain('POST /api/v1/agent/assets')
    expect(text).toContain('所有已认证分组')
    ...
    expect(text).toContain('结果转存与 Jingyu 回调')
    expect(text).toContain('本站转存后的 /media/* 结果地址')
  ...
  it('keeps the English Seedance documentation equally complete', () => {
    ...
    expect(text).toContain('Local Media Upload And Automatic Request Rewriting')
    expect(text).toContain('Every authenticated group')
    ...
    expect(text).toContain('Result rehosting and Jingyu callback')
    expect(text).toContain('The rehosted Sub2API /media/* result URL')
    expect(text).not.toContain('https://cdn.example.com/output.mp4')
  })
})
