      ...
      <template #table>
        ...
        >
          ...
          <template #cell-tokens="{ row }">
            <!-- 视频生成请求 -->
            <div v-if="isVideoUsage(row)" class="space-y-1 text-sm">
              <div class="flex items-center gap-1.5">
                <Icon name="play" size="sm" class="text-cyan-500" />
                <span class="font-medium text-gray-900 dark:text-white">{{ formatVideoDurationParts(row) }}</span>
                <span v-if="row.video_resolution" class="text-gray-400">({{ row.video_resolution }})</span>
              </div>
              <div class="max-w-[260px] truncate text-xs text-gray-500 dark:text-gray-400" :title="row.video_task_id || ''">
                {{ row.video_task_id || '-' }}
              </div>
            </div>
            <!-- 图片生成请求 -->
        ...
        <div class="space-y-1.5">
          ...
          <div class="mb-2 border-b border-gray-700 pb-1.5">
            ...
            <template v-else-if="tooltipData && isVideoUsage(tooltipData)">
              <div class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.videoTaskId') }}</span>
                <span class="max-w-[220px] truncate font-medium text-white">{{ tooltipData.video_task_id || '-' }}</span>
              </div>
              <div class="flex items-center justify-between gap-4">
              ...
              <div class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.videoGeneratedSeconds') }}</span>
                <span class="font-medium text-white">{{ tooltipData.video_duration_seconds || 0 }}s</span>
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.videoReferenceSeconds') }}</span>
                <span class="font-medium text-white">{{ tooltipData.video_reference_duration_seconds || 0 }}s</span>
              </div>
              <div class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.videoBillableSeconds') }}</span>
                <span class="font-medium text-white">{{ tooltipData.video_billable_seconds || 0 }}s</span>
              </div>
              <div class="flex items-center justify-between gap-4">
              ...
              <div v-if="tooltipData.video_result_url" class="flex items-center justify-between gap-4">
                <span class="text-gray-400">{{ t('usage.videoResultUrl') }}</span>
                <span class="max-w-[220px] truncate font-medium text-white">{{ tooltipData.video_result_url }}</span>
              </div>
            </template>
            <div v-else class="flex items-center justify-between gap-4">
...
const getRequestTypeBadgeClass = (log: UsageLog): string => {
  ...
  if (requestType === 'video') return 'bg-cyan-100 text-cyan-800 dark:bg-cyan-900 dark:text-cyan-200'
...
const getUsageBillingModeLabel = (log: UsageLog): string => {
  if (isVideoRefundUsage(log)) return t('usage.asyncTaskRefund')
  return getBillingModeLabel(getDisplayBillingMode(log), t)
}

const getUsageBillingModeBadgeClass = (log: UsageLog): string => {
  if (isVideoRefundUsage(log)) {
    return 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-300'
  }
  return getBillingModeBadgeClass(getDisplayBillingMode(log))
}
