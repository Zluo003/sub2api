...
const messages: Record<string, string> = {
  ...
  'usage.video': 'Video',
  'usage.videoTaskId': 'Video task ID',
  'usage.videoResolution': 'Resolution',
  'usage.videoGeneratedSeconds': 'Generated seconds',
  'usage.videoReferenceSeconds': 'Reference seconds',
  'usage.videoBillableSeconds': 'Billable seconds',
  'usage.videoUnitPrice': 'Per-second price',
  'usage.videoResultUrl': 'Result URL',
  'admin.usage.billingModeToken': 'Token',
  'admin.usage.billingModePerRequest': 'Per request',
  'admin.usage.billingModeImage': 'Image',
  'admin.usage.billingModeVideo': 'Per video',
  // billing_mode='video_duration' 现在有独立的按秒计费文案（见 utils/billingMode.ts）
  'admin.usage.billingModeVideoDuration': 'Video seconds',
...
describe('admin UsageTable tooltip', () => {
  ...
  it('shows video duration usage metadata without upstream fields', async () => {
    const wrapper = mount(UsageTable, {
      props: {
        data: [
          {
            ...
            request_type: 'video',
            image_count: 0,
            video_task_id: 'video_local_123',
            video_resolution: '720p',
            video_duration_seconds: 8,
            video_reference_duration_seconds: 12,
            video_billable_seconds: 20,
            video_result_url: 'https://cdn.example.com/out.mp4',
          },
        ],
        loading: false,
        columns: [],
    ...
    const text = wrapper.text()
    expect(text).toContain('Video seconds')
    expect(text).toContain('8s + 12s = 20s')
    expect(text).toContain('video_local_123')
    expect(text).toContain('Per-second price')
    expect(text).not.toContain('upstream_task_id')
    expect(text.toLowerCase()).not.toContain('aigod')
  })
})
