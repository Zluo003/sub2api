...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	agentModelRepository := repository.NewAgentModelRepository(db)
	agentModelCatalogService := service.NewAgentModelCatalogService(accountRepository, groupRepository, agentModelRepository)
	modelPricingResolver := service.ProvideModelPricingResolver(channelService, billingService, agentModelCatalogService)
	...
	groupHandler := admin.NewGroupHandler(adminService, dashboardService, groupCapacityService, agentModelCatalogService)
	...
	fileStorageService := service.NewFileStorageService(db, settingRepository, secretEncryptor, backupObjectStoreFactory, configConfig)
	fileStorageHandler := admin.NewFileStorageHandler(fileStorageService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, fileStorageHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, complianceHandler)
	usageRecordWorkerPool := service.NewUsageRecordWorkerPool(configConfig)
	userMsgQueueCache := repository.NewUserMsgQueueCache(redisClient)
	userMessageQueueService := service.ProvideUserMessageQueueService(userMsgQueueCache, rpmCache, configConfig)
	gatewayHandler := handler.NewGatewayHandler(gatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, agentModelCatalogService)
	temporaryAssetPublisher := service.NewTemporaryAssetPublisher(db, fileStorageService)
	openAIGatewayHandler := handler.NewOpenAIGatewayHandler(openAIGatewayService, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, temporaryAssetPublisher, configConfig)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, videoHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, agentHandler, idempotencyCoordinator, idempotencyCleanupService)
