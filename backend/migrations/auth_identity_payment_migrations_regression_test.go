...
func TestMigration159MergesVideoCompletionUsageRows(t *testing.T) {
	content, err := FS.ReadFile("159_video_usage_log_result_merge.sql")
	require.NoError(t, err)

	sql := string(content)
	require.Contains(t, sql, "UPDATE usage_logs charge")
	require.Contains(t, sql, "SET video_result_url = v.result_video_url")
	require.Contains(t, sql, "charge.request_id = 'video:' || v.public_id")
	require.Contains(t, sql, "DELETE FROM usage_logs ul")
	require.Contains(t, sql, "ul.request_id LIKE 'video:%:completed'")
	require.Contains(t, sql, "COALESCE(ul.actual_cost, 0) = 0")
	require.Contains(t, sql, "COALESCE(ul.total_cost, 0) = 0")
}

func TestMigration160BackfillsVideoEndpointAndDuration(t *testing.T) {
	content, err := FS.ReadFile("160_video_usage_log_endpoint_duration.sql")
	require.NoError(t, err)

	sql := string(content)
	require.Contains(t, sql, "UPDATE usage_logs ul")
	require.Contains(t, sql, "inbound_endpoint")
	require.Contains(t, sql, "upstream_endpoint")
	require.Contains(t, sql, "duration_ms")
	require.Contains(t, sql, "'/v1/videos'")
	require.Contains(t, sql, "FROM video_tasks v")
	require.Contains(t, sql, "v.status IN ('completed', 'failed', 'cancelled')")
}
