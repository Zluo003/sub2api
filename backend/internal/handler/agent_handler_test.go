...
func TestPreflightMediaOverridesCallerMetadataFromOwnedTemporaryAsset(t *testing.T) {
	gin.SetMode(gin.TestMode)
	h, mock := newAgentHandlerMock(t)
	...
	mock.ExpectQuery("SELECT media_type,mime_type,size_bytes,metadata").
		...
		WillReturnRows(sqlmock.NewRows([]string{"media_type", "mime_type", "size_bytes", "metadata"}).
			AddRow("image", "image/png", int64(4096), []byte(`{"width":1920,"height":1080,"probe":"go-image"}`)))
	r := gin.New()
	r.POST("/preflight", func(c *gin.Context) {
		c.Set(string(middleware.ContextKeyAPIKey), &service.APIKey{ID: 81})
		h.PreflightMedia(c)
	})
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/preflight", strings.NewReader(`{
		"model":"seedance-2.0","mode":"video_reference_to_video","prompt":"move",
		"duration_seconds":5,"ratio":"16:9","resolution":"1080P",
		"references":[{
			"temporary_asset_id":"`+temporaryID.String()+`","role":"reference_image",
			"media_type":"video","mime_type":"text/html","size_bytes":1,"width":1,"height":1
		}]
	}`))
	req.Header.Set("Content-Type", "application/json")
	r.ServeHTTP(w, req)
