...
func TestUpdateUserPlatformQuotas_Success(t *testing.T) {
	...
	// 缓存失效：请求中 2 个 platform + 软删除的 4 个 platform（gemini, antigravity, grok, seedance）= 6 次
	if len(cache.deleteCalls) != 6 {
		t.Errorf("expected 6 cache delete calls, got %d: %+v", len(cache.deleteCalls), cache.deleteCalls)
	}
}
