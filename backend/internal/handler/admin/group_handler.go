...
type CreateGroupRequest struct {
	...
	Platform         string             `json:"platform" binding:"omitempty,oneof=anthropic openai gemini antigravity grok seedance"`
	...
	VideoPricingRules           []service.VideoGroupPricingRule           `json:"video_pricing_rules"`
	// 分组 RPM 上限（0 = 不限制）
...
func (h *GroupHandler) Create(c *gin.Context) {
	...
	group, err := h.adminService.CreateGroup(c.Request.Context(), &service.CreateGroupInput{
		...
		VideoPricingRules:               req.VideoPricingRules,
