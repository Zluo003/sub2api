...
func (h *OpenAIGatewayHandler) GrokVideoStatus(c *gin.Context) {
	h.handleGrokMedia(c, service.GrokMediaEndpointVideoStatus, grokVideoRequestID(c))
}
...
func (h *OpenAIGatewayHandler) GrokVideoContent(c *gin.Context) {
	h.handleGrokMedia(c, service.GrokMediaEndpointVideoContent, grokVideoRequestID(c))
}
...
// grokVideoRequestID reads the video id from either route parameter name.
// 本仓库的 /v1/videos/:id（Seedance 异步任务）与上游 Grok 的 /videos/:request_id
// 是同一条路径，gin 不允许同段使用不同参数名，因此统一注册为 :id，
// 这里同时兼容两种命名。
func grokVideoRequestID(c *gin.Context) string {
	if v := c.Param("request_id"); v != "" {
		return v
	}
	return c.Param("id")
}
