...
type ResolvedPricing struct {
	...
	// PricingGroupID and ChannelID identify the concrete source used for Agent pricing.
...
func (r *ModelPricingResolver) ResolveAgentAccount(ctx context.Context, agentGroupID int64, account *Account, model string) (*ResolvedPricing, error) {
	...
	resolved := &ResolvedPricing{Mode: mode, Source: PricingSourceChannel, PricingGroupID: match.GroupID, ChannelID: match.ChannelID, SupportsCacheBreakdown: pricing.CacheWritePrice != nil || pricing.CacheReadPrice != nil, channelPricing: pricing}
	if mode == BillingModePerRequest || mode == BillingModeImage || mode == BillingModeVideo {
