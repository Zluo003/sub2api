...
func TestGeminiOAuthService_GenerateAuthURL_RedirectURIStrategy(t *testing.T) {
	...
	tests := []testCase{
		{
			name: "google_one requires configured OAuth client",
			cfg: &config.Config{
			...
			wantErrSubstr: "OAuth client not configured",
		},
		{
			name: "google_one uses configured OAuth client",
			cfg: &config.Config{
				Gemini: config.GeminiConfig{
					OAuth: config.GeminiOAuthConfig{
		...
		{
			name: "code_assist uses configured OAuth client",
			cfg: &config.Config{
				Gemini: config.GeminiConfig{
					OAuth: config.GeminiOAuthConfig{
						ClientID:     "custom-client-id",
		...
		{
			name: "ai_studio requires configured OAuth client",
			cfg: &config.Config{
				Gemini: config.GeminiConfig{
					OAuth: config.GeminiOAuthConfig{},
				},
