...
func (s *BillingCacheService) RollbackUserPlatformQuotaUsage(ctx context.Context, userID int64, platform string, cost float64) {
	...
	if s.cache != nil {
		...
		if err := s.cache.DeleteUserPlatformQuotaCache(cacheCtx, userID, platform); err != nil {
			logger.LegacyPrintf("service.billing_cache", "Warning: delete user platform quota cache failed user=%d platform=%s: %v", userID, platform, err)
		}
		cancel()
	}
	...
	dbCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), cacheWriteTimeout)
	defer cancel()
	var err error
	now := time.Now().UTC()
	if rollbacker, ok := s.userPlatformQuotaRepo.(interface {
		RollbackUsage(context.Context, int64, string, float64, time.Time) error
	}); ok {
		err = rollbacker.RollbackUsage(dbCtx, userID, platform, cost, now)
	} else {
		err = s.userPlatformQuotaRepo.IncrementUsageWithReset(dbCtx, userID, platform, -cost, now)
	}
	if err != nil {
		logger.LegacyPrintf("service.billing_cache", "Warning: rollback user platform quota usage failed user=%d platform=%s cost=%f: %v", userID, platform, cost, err)
	}
}
