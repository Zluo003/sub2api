...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if isVideoUsage {
		...
		// video_duration_seconds 列是 NOT NULL DEFAULT 0，UsageLog 里是值类型 int
		usageLog.VideoDurationSeconds = NormalizeVideoBillingDurationSecondsOrDefault(result.VideoDurationSeconds)
	}
	if cost != nil {
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	// 系统 Agent 分组：按实际选中账号所属平台的源渠道价 × 该平台倍率计费，
	// 不走本分组自身的渠道价与用户倍率。
	if apiKey.Group != nil && apiKey.Group.IsAgent() {
		if s.resolver == nil {
