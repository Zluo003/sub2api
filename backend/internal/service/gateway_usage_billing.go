...
// 下面三个 *Command 判定用于统一计费命令路径，与上面 legacy 判定的区别是用
// "!= 0" 而不是 "> 0"：视频任务失败退款会写入负数成本的用量行，必须让退款同样
// 回滚 API Key 配额、限流计数与账号配额，否则退款只退余额、配额永远扣着不还。
func (p *postUsageBillingParams) shouldApplyAPIKeyQuotaCommand() bool {
	return p.Cost.ActualCost != 0 && p.APIKey.Quota > 0 && p.APIKeyService != nil
}

func (p *postUsageBillingParams) shouldApplyRateLimitCommand() bool {
	return p.Cost.ActualCost != 0 && p.APIKey.HasRateLimits() && p.APIKeyService != nil
}

func (p *postUsageBillingParams) shouldApplyAccountQuotaCommand() bool {
	return p.Cost.TotalCost != 0 && p.Account.IsAPIKeyOrBedrock() && p.Account.HasAnyQuotaLimit()
}
...
func buildUsageBillingCommand(requestID string, usageLog *UsageLog, p *postUsageBillingParams) *UsageBillingCommand {
	...
	// 用 "!= 0" 而不是 "> 0"：视频任务失败退款写入的是负数成本，退款行必须同样
	// 走订阅/余额回滚分支，否则退款会被这里直接丢弃。
	...
	if p.shouldApplyAPIKeyQuotaCommand() {
		cmd.APIKeyQuotaCost = p.Cost.ActualCost
	}
	if p.shouldApplyRateLimitCommand() {
		cmd.APIKeyRateLimitCost = p.Cost.ActualCost
	}
	if p.shouldApplyAccountQuotaCommand() {
...
func applyUsageBilling(ctx context.Context, requestID string, usageLog *UsageLog, p *postUsageBillingParams, deps *billingDeps, repo UsageBillingRepository) (bool, error) {
	...
	if result == nil || !result.Applied {
		// 视频链路可以在没有 deferredService 的情况下计费，这里保持 nil 安全。
...
type billingDeps struct {
	// 只需要 IncrementQuotaUsed，收窄到 AccountQuotaUsageRepository 后
	// VideoService 的 VideoAccountRepository 也能直接复用这套计费依赖。
	accountRepo           AccountQuotaUsageRepository
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	// Agent 分组使用独立的渠道定价，不应用普通分组的高峰倍率。
	pricingAt := input.PricingAt
	if pricingAt.IsZero() {
...
// calculateAgentRecordUsageCost 计算系统 Agent 分组的最终计费（图片按张、其余按 token）。
// 上游拆分 gateway_service.go 后本方法迁移到这里。
