...
type TotpConfig struct {
	...
	// 如果为空，启动时会生成并在数据库引导阶段持久化（仅适用于开发环境）
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	if cfg.Totp.EncryptionKey == "" {
		...
		slog.Warn("TOTP encryption key auto-generated; it will be persisted during database bootstrap. Consider setting a managed key for production.")
	} else {
