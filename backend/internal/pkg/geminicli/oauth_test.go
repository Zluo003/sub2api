...
const (
	testGeminiOAuthClientID     = "test-gemini-oauth-client-id"
	testGeminiOAuthClientSecret = "test-gemini-oauth-client-secret"
)
...
func TestBuildAuthorizationURL(t *testing.T) {
	authURL, err := BuildAuthorizationURL(
		OAuthConfig{ClientID: testGeminiOAuthClientID, ClientSecret: testGeminiOAuthClientSecret},
		"test-state",
		"test-challenge",
	...
	checks := []string{
		"response_type=code",
		"client_id=" + testGeminiOAuthClientID,
		"redirect_uri=",
		"state=test-state",
...
func TestBuildAuthorizationURL_MissingOAuthClient(t *testing.T) {
	_, err := BuildAuthorizationURL(
		OAuthConfig{},
		"test-state",
		"test-challenge",
	...
	if err == nil {
		t.Fatal("缺少 OAuth client 时应该报错")
	}
	if !strings.Contains(err.Error(), "OAuth client not configured") {
		t.Errorf("错误消息应包含 OAuth client not configured，实际: %v", err)
	}
}
...
func TestEffectiveOAuthConfig_GoogleOne(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Google One 自定义客户端 + 自定义 scopes",
			input: OAuthConfig{
				ClientID:     testGeminiOAuthClientID,
				ClientSecret: testGeminiOAuthClientSecret,
			...
			oauthType:    "google_one",
			wantClientID: testGeminiOAuthClientID,
			wantScopes:   "https://www.googleapis.com/auth/cloud-platform https://www.googleapis.com/auth/generative-language.retriever https://www.googleapis.com/auth/drive.readonly",
			wantErr:      false,
		},
		{
...
func TestEffectiveOAuthConfig_CustomScopesArePreserved(t *testing.T) {
	...
	if !strings.Contains(cfg.Scopes, "generative-language.retriever") {
		t.Errorf("Scopes 应保留 generative-language.retriever，实际: %v", cfg.Scopes)
	}
	if !strings.Contains(cfg.Scopes, "drive.readonly") {
		t.Errorf("Scopes 应保留 drive.readonly，实际: %v", cfg.Scopes)
	}
}
...
func TestEffectiveOAuthConfig_CodeAssist_DefaultScopes_CustomClient(t *testing.T) {
	cfg, err := EffectiveOAuthConfig(OAuthConfig{
		ClientID:     "custom-id",
		ClientSecret: "custom-secret",
	}, "code_assist")
	...
	if cfg.Scopes != DefaultCodeAssistScopes {
		t.Errorf("code_assist + 自定义客户端应使用 DefaultCodeAssistScopes，实际: %q", cfg.Scopes)
	}
}
...
func TestEffectiveOAuthConfig_NoOAuthClient(t *testing.T) {
