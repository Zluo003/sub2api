...
const (
	testAntigravityClientID     = "test-antigravity-client-id"
	testAntigravityClientSecret = "test-antigravity-client-secret"
)

func configureTestAntigravityOAuthClient(t *testing.T, clientID, clientSecret string) {
	t.Helper()
...
func TestGetClientSecret_已配置(t *testing.T) {
	old := defaultClientSecret
	defaultClientSecret = "my-secret-value"
...
func TestGetClientSecret_环境变量为空(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "client_secret") {
		t.Errorf("错误信息应包含 client_secret: got %s", err.Error())
	}
}
...
func TestBuildAuthorizationURL_参数验证(t *testing.T) {
	configureTestAntigravityOAuthClient(t, testAntigravityClientID, testAntigravityClientSecret)
