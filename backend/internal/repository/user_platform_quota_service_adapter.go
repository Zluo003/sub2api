...
func (a *userPlatformQuotaServiceAdapter) RollbackUsage(ctx context.Context, userID int64, platform string, cost float64, now time.Time) error {
	return a.inner.RollbackUsage(ctx, userID, platform, cost, now)
}
...
func (a *genericUserPlatformQuotaAdapter) RollbackUsage(ctx context.Context, userID int64, platform string, cost float64, now time.Time) error {
	if rollbacker, ok := a.inner.(interface {
		RollbackUsage(context.Context, int64, string, float64, time.Time) error
	}); ok {
		return rollbacker.RollbackUsage(ctx, userID, platform, cost, now)
	}
	return nil
}
