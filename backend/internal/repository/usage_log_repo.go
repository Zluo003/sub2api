...
var usageLogInsertArgTypes = [...]string{
	...
	"integer",     // video_reference_duration_seconds
	"integer",     // video_billable_seconds
	"text",        // video_result_url
...
func (r *usageLogRepository) UpdateVideoResult(ctx context.Context, requestID string, apiKeyID int64, update service.VideoUsageResultUpdate) error {
	requestID = strings.TrimSpace(requestID)
	resultURL := strings.TrimSpace(update.ResultURL)
	inboundEndpoint := strings.TrimSpace(update.InboundEndpoint)
	upstreamEndpoint := strings.TrimSpace(update.UpstreamEndpoint)
	...
	if update.DurationMs != nil {
		durationArg = *update.DurationMs
	}
	_, err := exec.ExecContext(ctx, `
		UPDATE usage_logs
		SET video_result_url = COALESCE(NULLIF($3, ''), video_result_url),
		    duration_ms = COALESCE($4, duration_ms),
		    inbound_endpoint = COALESCE(NULLIF($5, ''), inbound_endpoint),
		    upstream_endpoint = COALESCE(NULLIF($6, ''), upstream_endpoint)
		WHERE request_id = $1
		  AND api_key_id = $2
		  AND billing_mode = 'video_duration'
		  AND video_task_id IS NOT NULL
	`, requestID, apiKeyID, resultURL, durationArg, inboundEndpoint, upstreamEndpoint)
	return err
}
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	videoTaskID := nullString(log.VideoTaskID)
	videoResolution := nullString(log.VideoResolution)
	videoResultURL := nullString(log.VideoResultURL)
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			log.VideoReferenceDurationSeconds,
			log.VideoBillableSeconds,
			videoResultURL,
			createdAt,
		},
	}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		videoReferenceSeconds int
		videoBillableSeconds  int
		videoResultURL        sql.NullString
		createdAt             time.Time
	)
	...
	log := &service.UsageLog{
		...
		VideoReferenceDurationSeconds: videoReferenceSeconds,
		VideoBillableSeconds:          videoBillableSeconds,
	...
	if videoTaskID.Valid {
		log.VideoTaskID = &videoTaskID.String
	}
	if videoResolution.Valid {
		log.VideoResolution = &videoResolution.String
	}
	if videoResultURL.Valid {
		log.VideoResultURL = &videoResultURL.String
	}
