...
func (r *usageBillingRepository) applyUsageBillingEffects(ctx context.Context, tx *sql.Tx, cmd *service.UsageBillingCommand, result *service.UsageBillingApplyResult) error {
	if cmd.SubscriptionCost > 0 && cmd.SubscriptionID != nil {
		...
		if err := refundUsageBillingSubscription(ctx, tx, *cmd.SubscriptionID, -cmd.SubscriptionCost); err != nil {
			return err
		}
	}
	...
	if cmd.APIKeyRateLimitCost > 0 {
		...
		if err := refundUsageBillingAPIKeyRateLimit(ctx, tx, cmd.APIKeyID, -cmd.APIKeyRateLimitCost); err != nil {
			return err
		}
	}
	...
	if cmd.AccountQuotaCost > 0 && (strings.EqualFold(cmd.AccountType, service.AccountTypeAPIKey) || strings.EqualFold(cmd.AccountType, service.AccountTypeBedrock)) {
		...
		quotaState, err := refundUsageBillingAccountQuota(ctx, tx, cmd.AccountID, -cmd.AccountQuotaCost)
		if err != nil {
			return err
		}
		result.QuotaState = quotaState
...
func refundUsageBillingSubscription(ctx context.Context, tx *sql.Tx, subscriptionID int64, costUSD float64) error {
	const updateSQL = `
		UPDATE user_subscriptions us
		SET
			daily_usage_usd = GREATEST(us.daily_usage_usd - $1, 0),
			weekly_usage_usd = GREATEST(us.weekly_usage_usd - $1, 0),
			monthly_usage_usd = GREATEST(us.monthly_usage_usd - $1, 0),
			updated_at = NOW()
		FROM groups g
		WHERE us.id = $2
			AND us.deleted_at IS NULL
...
func refundUsageBillingAPIKeyRateLimit(ctx context.Context, tx *sql.Tx, apiKeyID int64, cost float64) error {
	res, err := tx.ExecContext(ctx, `
		UPDATE api_keys SET
			usage_5h = GREATEST(usage_5h - $1, 0),
			usage_1d = GREATEST(usage_1d - $1, 0),
			usage_7d = GREATEST(usage_7d - $1, 0),
			updated_at = NOW()
		WHERE id = $2 AND deleted_at IS NULL
	`, cost, apiKeyID)
	if err != nil {
...
func refundUsageBillingAccountQuota(ctx context.Context, tx *sql.Tx, accountID int64, amount float64) (*service.AccountQuotaState, error) {
	rows, err := tx.QueryContext(ctx,
		`UPDATE accounts SET extra = (
			COALESCE(extra, '{}'::jsonb)
			|| jsonb_build_object('quota_used', GREATEST(COALESCE((extra->>'quota_used')::numeric, 0) - $1, 0))
			|| jsonb_build_object('quota_daily_used', GREATEST(COALESCE((extra->>'quota_daily_used')::numeric, 0) - $1, 0))
			|| jsonb_build_object('quota_weekly_used', GREATEST(COALESCE((extra->>'quota_weekly_used')::numeric, 0) - $1, 0))
		), updated_at = NOW()
		WHERE id = $2 AND deleted_at IS NULL
		RETURNING
			COALESCE((extra->>'quota_used')::numeric, 0),
	...
	if err := enqueueSchedulerOutbox(ctx, tx, service.SchedulerOutboxEventAccountChanged, &accountID, nil, nil); err != nil {
		logger.LegacyPrintf("repository.usage_billing", "[SchedulerOutbox] enqueue quota refund failed: account=%d err=%v", accountID, err)
		return nil, err
	}
	return &state, nil
}
