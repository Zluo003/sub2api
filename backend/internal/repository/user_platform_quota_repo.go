...
func (r *userPlatformQuotaRepository) RollbackUsage(ctx context.Context, userID int64, platform string, cost float64, now time.Time) error {
	if cost <= 0 || strings.TrimSpace(platform) == "" {
		return nil
	}
	client := clientFromContext(ctx, r.client)
	_, err := client.ExecContext(ctx, `
		...
		SET
			daily_usage_usd = CASE
				WHEN daily_window_start IS NULL OR daily_window_start < $4 THEN daily_usage_usd
				ELSE GREATEST(daily_usage_usd - $3, 0)
			END,
			weekly_usage_usd = CASE
				WHEN weekly_window_start IS NULL OR weekly_window_start < $5 THEN weekly_usage_usd
				ELSE GREATEST(weekly_usage_usd - $3, 0)
			END,
			monthly_usage_usd = CASE
				WHEN monthly_window_start IS NULL OR monthly_window_start + INTERVAL '30 days' <= $6 THEN monthly_usage_usd
				ELSE GREATEST(monthly_usage_usd - $3, 0)
			END,
			updated_at = $6
		WHERE user_id = $1
			AND platform = $2
