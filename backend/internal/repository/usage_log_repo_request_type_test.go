...
func appendDefaultVideoUsageInsertArgs(args ...driver.Value) []driver.Value {
	...
	createdAt := args[len(args)-1]
	out := append([]driver.Value{}, args[:len(args)-1]...)
	out = append(out,
		sqlmock.AnyArg(), // video_task_id
		sqlmock.AnyArg(), // video_resolution
		0,                // video_duration_seconds
...
func appendDefaultVideoUsageScanValues(values ...any) []any {
	...
	createdAt := values[len(values)-1]
	out := append([]any{}, values[:len(values)-1]...)
	out = append(out,
