...
func TestUsageLogRepositoryCreateSyncRequestTypeAndLegacyFields(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO usage_logs").
		WithArgs(appendDefaultVideoUsageInsertArgs(
			log.UserID,
			log.APIKeyID,
			log.AccountID,
			log.RequestID,
...
func TestUsageLogRepositoryUpdateVideoResult(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := &usageLogRepository{sql: db}
	durationMs := 42000

	mock.ExpectExec("UPDATE usage_logs").
		WithArgs("video:task-1", int64(2), "https://cdn.example.com/output.mp4", durationMs, "/v1/videos", "/v1/videos").
		WillReturnResult(sqlmock.NewResult(0, 1))

	err := repo.UpdateVideoResult(context.Background(), " video:task-1 ", 2, service.VideoUsageResultUpdate{
...
// 本仓库的 4 个视频列紧跟在上游的 video_duration_seconds 之后，
// 其后固定还有 13 列（service_tier … created_at），据此就地插入。
const trailingUsageLogInsertColumns = 13

func appendDefaultVideoUsageInsertArgs(args ...driver.Value) []driver.Value {
	if len(args) <= trailingUsageLogInsertColumns {
		return args
	}
	at := len(args) - trailingUsageLogInsertColumns
	out := append([]driver.Value{}, args[:at]...)
	out = append(out,
		sqlmock.AnyArg(), // video_task_id
		0,                // video_reference_duration_seconds
		0,                // video_billable_seconds
		sqlmock.AnyArg(), // video_result_url
	)
	return append(out, args[at:]...)
}

func appendDefaultVideoUsageScanValues(values ...any) []any {
	if len(values) <= trailingUsageLogInsertColumns {
		return values
	}
	at := len(values) - trailingUsageLogInsertColumns
	out := append([]any{}, values[:at]...)
	out = append(out,
		sql.NullString{}, // video_task_id
		0,                // video_reference_duration_seconds
		0,                // video_billable_seconds
		sql.NullString{}, // video_result_url
	)
...
func TestScanUsageLogRequestTypeAndLegacyFallback(t *testing.T) {
	t.Run("image_size_metadata_is_scanned", func(t *testing.T) {
		now := time.Now().UTC()
		log, err := scanUsageLog(usageLogScannerStub{values: appendDefaultVideoUsageScanValues(
			int64(4),
			int64(13),
			int64(23),
			int64(33),
			...
			0,                // video_duration_seconds（本仓库该列 NOT NULL，扫描目标是 int）
			sql.NullString{},
			sql.NullString{},
			sql.NullString{},
			sql.NullString{},
