...
func TestMigrationsRunner_IsIdempotent_AndSchemaIsUpToDate(t *testing.T) {
	...
	requireColumn(t, tx, "usage_logs", "video_task_id", "character varying", 64, true)
	requireColumn(t, tx, "usage_logs", "video_resolution", "character varying", 16, true)
	requireColumn(t, tx, "usage_logs", "video_duration_seconds", "integer", 0, false)
	requireColumn(t, tx, "usage_logs", "video_reference_duration_seconds", "integer", 0, false)
	requireColumn(t, tx, "usage_logs", "video_billable_seconds", "integer", 0, false)
	requireColumn(t, tx, "usage_logs", "video_result_url", "text", 0, true)
	...
	var videoTasksRegclass sql.NullString
	require.NoError(t, tx.QueryRowContext(context.Background(), "SELECT to_regclass('public.video_tasks')").Scan(&videoTasksRegclass))
	require.True(t, videoTasksRegclass.Valid, "expected video_tasks table to exist")
	requireColumn(t, tx, "video_tasks", "refunded_at", "timestamp with time zone", 0, true)
