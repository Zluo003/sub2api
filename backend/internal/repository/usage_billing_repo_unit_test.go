...
const (
	...
	balanceRefundSQL            = `(?s)UPDATE users\s+SET balance = balance \+ \$1,\s+updated_at = NOW\(\)\s+WHERE id = \$2 AND deleted_at IS NULL\s+RETURNING balance`
...
func TestApplyUsageBillingEffects_RefundsNegativeCosts(t *testing.T) {
	ctx := context.Background()
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	...
	tx, err := db.BeginTx(ctx, nil)
	require.NoError(t, err)
	mock.ExpectQuery(balanceRefundSQL).
		WithArgs(4.0, int64(42)).
		WillReturnRows(sqlmock.NewRows([]string{"balance"}).AddRow(104.0))
	mock.ExpectExec(`(?s)UPDATE api_keys\s+SET quota_used = GREATEST\(quota_used - \$1, 0\),`).
		WithArgs(4.0, int64(7), service.StatusAPIKeyQuotaExhausted, service.StatusAPIKeyActive).
	...
	require.NoError(t, err)
	require.NotNil(t, result.NewBalance)
	require.InDelta(t, 104.0, *result.NewBalance, 0.000001)
	require.NoError(t, tx.Commit())
	require.NoError(t, mock.ExpectationsWereMet())
}
