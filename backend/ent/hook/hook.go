...
// The VideoGroupPricingRuleFunc type is an adapter to allow the use of ordinary
// function as VideoGroupPricingRule mutator.
type VideoGroupPricingRuleFunc func(context.Context, *ent.VideoGroupPricingRuleMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f VideoGroupPricingRuleFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.VideoGroupPricingRuleMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.VideoGroupPricingRuleMutation", m)
}
